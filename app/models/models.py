...
class Cart(db.Model):
    ...
    branch = relationship('Branch', backref='carts')

#Create table to store order
class Order(db.Model):
    __tablename__ = 'order'
    order_id = db.Column(db.Integer, primary_key=True, nullable=False)
    username = db.Column(db.Integer, db.ForeignKey('user.username'), nullable=False)#needs to be string
    ...
    branch = relationship('Branch', backref='order')
...
class User(db.Model, UserMixin):
    ...
    branch = relationship('Branch', backref='user')
...
class Assets(db.Model):
    ...
    asset_name = db.Column(db.String(100), nullable=True)#change to not null
...
class Branch(db.Model):
    ...
    address_line2 = db.Column(db.String(225), nullable=True)
